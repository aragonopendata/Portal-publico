...
-->
<div class="en-linea">
	<div class="container text-center aside-block">
		<span class="h2">${.vars['reserved-article-title'].data}</span>
				...
				<div class="aside-block-module">
					<div class="aside-block-module__title">
						...
						<#if cur_sectionTitle.linkTitle.getSiblings()?has_content && cur_sectionTitle.linkTitle.getSiblings()[0].getData()?? && cur_sectionTitle.linkTitle.getSiblings()[0].getData() != "">
							<a href="javascript:void(0)" data-control-collapse="${.vars['reserved-article-id'].data}-help-section-v-${cur_sectionTitle?index}" aria-expanded="false" class="name-container with-content">
						<#else>
					...
					<#if cur_sectionTitle.linkTitle.getSiblings()?has_content && cur_sectionTitle.linkTitle.getSiblings()[0].getData()?? && cur_sectionTitle.linkTitle.getSiblings()[0].getData() != "">
						<ul class="aside-block-module__tramites collapse" data-content-collapse="${.vars['reserved-article-id'].data}-help-section-v-${cur_sectionTitle?index}" id="${.vars['reserved-article-id'].data}-help-section-v-${cur_sectionTitle?index}">
