<style>
	...
	.dga-view .mapa-web .listado__item p.link {
		margin: 0;
	}
	.dga-view .mapa-web .listado__item p.link span {
		cursor: default;
	} 
	.dga-view .mapa-web .listado__item p.link:hover .title {
...
<div class="container u-container-mobile-0">
	<div class="list-news-module mapa-web">
		<#if entries?has_content> 
			<ul class="listado">
				<!--Inicio--> 
				<li class="col-12 listado__item"> <a class="link" href="/"> 
					<span class="title"><@liferay.language key="aragon.home"/></span></a>
				</li>
				...
				<#if layoutLocalService.fetchLayoutByFriendlyURL(themeDisplay.getScopeGroupId(), false, "/temas")??>
					...
					<li class="col-12 listado__item">
						<a class="link" href="${temasLayout.getFriendlyURL()}"> <span class="title">${temasLayout.getName(locale)}</span> </a> 
						<#if childLayoutsLinksMap?? && childLayoutsLinksMap?has_content>
							<button class="arrow-open js-arrow-open collapsed" data-toggle="collapse" data-target="#collapse_${itemsCounter}" aria-expanded="false" aria-controls="collapse_${itemsCounter}" aria-label="<@liferay.language key="aragon.template.web-map.show-children"/> ${entry.getName()}" data-label-closed="<@liferay.language key="aragon.template.web-map.show-children"/> ${entry.getName()}" data-label-opened="<@liferay.language key="aragon.template.web-map.hidden-children"/> ${entry.getName()}"></button> 
							<div id="collapse_${itemsCounter}" class="collapse">
								<ul class="listado">
									<#list childLayoutsLinksMap?keys as childLayout> <#assign itemsCounter = itemsCounter + 1/> 
										<li class="col-12 listado__item">
					...
					<#if entry??>
						...
						<li class="col-12 listado__item">
							...
							<#if entryURL != themeDisplay.getPortalURL()>
								<a class="link" href="${entryURL}"> <span class="title">${entry.getName()}</span> </a> 
							<#else> 
								<p class="link"> <span class="title">${entry.getName()}</span> </p>
							</#if>
				...
				<#if searcherUrl?? && searcherUrl?has_content> 
					<li class="col-12 listado__item">
						<a class="link" href="${searcherUrl}">
							<span class="title"><@liferay.language key="aragon.template.web-map.search"/></span>
						</a>
					</li>
				</#if>
			</ul>
