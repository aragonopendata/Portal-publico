...
@Component(
		immediate = true,
		service = BuildMapsSchedulerListener.class
	)
public class BuildMapsSchedulerListener extends BaseMessageListener {
	private static final Log _log = LogFactoryUtil.getLog(BuildMapsSchedulerListener.class);
	//private static final String _DEFAULT_CRON_EXPRESSION = "0 0/2 * 1/1 * ? *"; //cada 10 minuto
	private static final String _DEFAULT_CRON_EXPRESSION = "0 0 0/1 ? * * *"; //cada hora
	...
	protected void doReceive(Message message) throws Exception {
		new SchedulerTaskBuildMaps().run();
	}
