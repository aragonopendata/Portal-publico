...
<%-- START - Hide title and back button and customize the actions menu --%>
<%--
<div class="h2">
	<c:if test="<%= assetPublisherDisplayContext.isShowAssetTitle() %>">
		<c:if test="<%= showBackURL && Validator.isNotNull(redirect) %>">
			<liferay-ui:icon cssClass="header-back-to" icon="angle-left" markupView="lexicon" url="<%= redirect %>"/>
		</c:if>
...
<%-- END - Hide title and back button and customize the actions menu --%>
