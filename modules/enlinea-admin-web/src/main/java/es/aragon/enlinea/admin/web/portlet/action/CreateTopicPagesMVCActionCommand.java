...
@Component(
	...
	property = {
		...
		"mvc.command.name=" + EnlineaAdminPortletKeys.ACTION_CREATE_TOPIC_PAGES
	},
	service = MVCActionCommand.class
)
public class CreateTopicPagesMVCActionCommand extends BaseMVCActionCommand {
	
	private static Log log = LogFactoryUtil.getLog(CreateTopicPagesMVCActionCommand.class);
	...
	protected void doProcessAction(ActionRequest actionRequest, ActionResponse actionResponse) {
		...
		if(level1PageTemplate != -1 && level23PageTemplate != -1) {
			...
			if(Validator.isNotNull(assetVocabulary)) {
				...
				SessionErrors.add(actionRequest, "action.error.no-topic-vocabulary");
			}
		} else {
