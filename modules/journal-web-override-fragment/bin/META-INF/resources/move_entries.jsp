			...
			<liferay-frontend:fieldset>
				...
				<c:choose>
					<c:when test="<%=!permissionChecker.isOmniadmin()%>">
						<%
						Map<String, String> listFolderNameMap = new LinkedHashMap<>();
						journalWebOverrideFragmentUtil.getJournalFolderSelectorOptions(themeDisplay, 0,listFolderNameMap);
						%>
						<aui:select name="folderName" title="Nueva carpeta">
							<aui:option value="0" label="Inicio"></aui:option>
							<% for (String idFolder : listFolderNameMap.keySet()){ %>
							<aui:option value="<%=idFolder %>" label="<%=listFolderNameMap.get(idFolder) %>"></aui:option>
							<% } %>
						</aui:select>
						<script>
							$('#<portlet:namespace/>folderName').on('change', function() {
								var idFolder = document.getElementById("<portlet:namespace/>folderName").value;
								$("#<portlet:namespace/>newFolderId").val(idFolder);
							});
						</script>
					</c:when>
					<c:otherwise>
