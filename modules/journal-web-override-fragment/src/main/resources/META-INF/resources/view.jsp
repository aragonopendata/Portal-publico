					...
					<c:choose>
						...
						<c:otherwise>
							...
							if (journalDisplayContext.hasResults()) {
								//START MODIFICATION - ADDING CUSTOM SEARCH CONTAINER TO FILTER SEARCH RESULTS
								int totalSearchResultsWithoutVersion = journalWebOverrideFragmentUtil.getSearchContainer(request, liferayPortletRequest, liferayPortletResponse, liferayPortletResponse.createRenderURL(), true, false).getTotal();
								String tabName = StringUtil.appendParentheticalSuffix(LanguageUtil.get(request, "web-content"), totalSearchResultsWithoutVersion);
								//END MODIFICATION - ADDING CUSTOM SEARCH CONTAINER TO FILTER SEARCH RESULTS
							...
							if (journalDisplayContext.hasVersionsResults()) {
								//START MODIFICATION - ADDING CUSTOM SEARCH CONTAINER TO FILTER SEARCH RESULTS
								int totalSearchResultsWithVersion = journalWebOverrideFragmentUtil.getSearchContainer(request, liferayPortletRequest, liferayPortletResponse, liferayPortletResponse.createRenderURL(), true, true).getTotal();
								String tabName = StringUtil.appendParentheticalSuffix(LanguageUtil.get(request, "versions"), totalSearchResultsWithVersion);
								//END MODIFICATION - ADDING CUSTOM SEARCH CONTAINER TO FILTER SEARCH RESULTS
								tabsNames = ArrayUtil.append(tabsNames, tabName);
