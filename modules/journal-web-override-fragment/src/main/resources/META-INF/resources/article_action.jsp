...
<%@page import="com.liferay.journal.model.JournalContentSearch"%>
...
/*----------CUSTOM CODE----------*/
String friendlyURL = "/-/" + article.getUrlTitle(locale);
String articleLayoutUrl = StringPool.BLANK;
if(Validator.isNotNull(article)){
	...
	if(Validator.isNotNull(journalContentSearchList) && journalContentSearchList.size() > 0) {
		...
		Layout articleLayout = LayoutLocalServiceUtil.fetchLayout(journalContentSearch.getGroupId(), journalContentSearch.getPrivateLayout(), journalContentSearch.getLayoutId());
		if(articleLayout.isPublicLayout()) {
			articleLayoutUrl = articleLayout.getFriendlyURL(locale);
		}
	}
}
/*----------END CUSTOM CODE----------*/
...
>
	...
	<c:if test="<%= JournalArticlePermission.contains(permissionChecker, article, ActionKeys.PERMISSIONS) && permissionChecker.isOmniadmin() %>">
	...
	<c:if test="<%= JournalArticlePermission.contains(permissionChecker, article, ActionKeys.VIEW) %>">
		...
		<!---------- CUSTOM CODE ---------->
		<c:if test="<%= friendlyURL != StringPool.BLANK && article.getLayoutUuid() != null  && article.getLayoutUuid() != StringPool.BLANK%>">
			<liferay-ui:icon
				target="_blank"
				message="aragon-view-in-page-button"
				url="<%= friendlyURL %>"
			/>
		</c:if>
		<!---------- END CUSTOM CODE ---------->
